...
const getRows = async () => {
	...
	const rows = await api.get('customer');
	renderTbody(rows);
	addChangeEvent('name');
...
const putRows = async () => {
	...
	for (let i = 0; i < id.length; i++) {
		...
		if (Object.keys(rowMap).some(key => rowMap[key].value !== rowBkMap[key].value)) {
			...
			try {
				const data = await api.put('customer', requestBody);
...
const postRow = async () => {
	...
	if (Object.keys(rowMap).some(key => rowMap[key].value !== '')) {
		...
		try {
			const data = await api.post('customer', requestBody);
			...
			Object.keys(rowMap).forEach(key => {
				rowMap[key].classList.toggle('error', key === e.details.field || `customer.${key}` === e.details.column);
			});
